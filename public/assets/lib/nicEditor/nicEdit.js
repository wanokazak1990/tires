...
var nicImgsMgrButton = nicEditorAdvancedButton.extend({addPane: function() {
		...
		var iconPic = item.thumb ? "url('" + path + ".thumbs/" + item.hash + "_thumb." + (item.type == "bmp" ? "jpg" : item.type) + "')" : "none";
		var li = new bkElement("DIV").setStyle({cursor: "pointer", clear: "both", margin: "0px 0px 0px 20px", padding: "1px 0px 1px 1px", fontSize: "12px", height: "37px"});
		var icon = new bkElement("DIV").setStyle({width: "36px", height: "36px", float: "left", margin: "0px 4px 0px 0", border: "1px solid #CCCCCC", backgroundColor: "transparent", backgroundImage: iconPic, backgroundRepeat: "no-repeat", backgroundPosition: "50% 50%"}).appendTo(li);
		if (iconPic == "none")
		...
		new bkElement("DIV").setStyle({padding: "2px 0px 1px", whiteSpace: "nowrap", overflow: "hidden"}).setContent(name).appendTo(li);
		new bkElement("DIV").setStyle({padding: "2px 0px 1px", color: "#888888", fontSize: "11px", fontStyle: "italic"}).setContent(info).appendTo(li);
		if (item.type == "folder")
			...
			li.addEvent("click", this.image.closure(this, path + name, name, item.preview ? item.preview_height : item.height, item.preview ? item.preview_width : item.width, (item.preview ? path + ".thumbs/" + item.hash + "." + item.type : false))).addEvent("mouseover", this.on.closure(this, li)).addEvent("mouseout", this.off.closure(this, li));
		...
		A.setStyle({backgroundColor: "#EEEEEE"})
	}, off: function(A) {
		A.setStyle({backgroundColor: "transparent"})
